...
collision_objects.append(House1(180, 85, 600, 600))
collision_objects.append(House2(174, 65, 600, 600))
collision_objects.append(House1(67, 60, 600, 600))
collision_objects.append(House2(78, 60, 600, 600))
collision_objects.append(House1(99, 60, 600, 600))
collision_objects.append(House1(110, 60, 600, 600))

# Создание объектов в соответствии с картой объектов
for row in range(len(objects_map)):
    for col in range(len(objects_map[row])):
        object_type = objects_map[row][col]
        if object_type == 1:
            collision_objects.append(Oak(col, row))
        elif object_type == 2:
            collision_objects.append(Spruce(col, row))
        # Добавьте другие условия для других типов объектов, если необходимо
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_x:
                ...
                numeric_objects = [[object_mapping[objects[row][col].__class__] if objects[row][col] else 0 for col in range(MAP_WIDTH * 2)] for row in range(MAP_HEIGHT * 2)]
                with open("objects_map.txt", "w") as file:
                    for row in numeric_objects:
                        file.write(" ".join(map(str, row)) + "\n")
        elif event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:  # Левая кнопка мыши
            # Получение координат мыши
            mouse_x, mouse_y = pygame.mouse.get_pos()
            # Пересчет координат в координаты карты
            map_col = int((mouse_x - offset_x) / (TILE_SIZE * current_scale))
            map_row = int((mouse_y - offset_y) / (TILE_SIZE * current_scale))
            # Создание Spruce объекта на месте клика
            if 0 <= map_row < MAP_HEIGHT and 0 <= map_col < MAP_WIDTH:
                collision_objects.append(House1(map_col, map_row, 600, 600))
