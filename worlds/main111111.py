...
class MainMenu:
    def __init__(self):
        ...
        self.fps = 100
        self.running = True
        self.is_character_editor = False
        self.is_options = False
        self.is_music = False
        self.button_sound = pygame.mixer.Sound('sound_effects/button_click_3.mp3')
        self.button_track_sound = pygame.mixer.Sound('sound_effects/button_tracking_04.mp3')
        self.main_menu_music = pygame.mixer.Sound('music/main_menu_music.mp3')
        self.normal_alpha = 255
        self.hover_alpha = 215
        self.pressed_alpha = 150
        self.disabled_alpha = 100

        with open('is_music.txt', mode='r', encoding='utf-8') as file:
            if file.read() == 'True':
                self.m_music = self.main_menu_music.play()
                self.is_music = True

        self.all_sprites = pygame.sprite.Group()
        self.background = Background(self.all_sprites)
        self.menu_back = Menu(self.all_sprites)
        self.new_game_button = NewGameButton(self.all_sprites)
        self.continue_game_button = ContinueGameButton(self.all_sprites)
        self.exit_button = ExitButton(self.all_sprites)
        self.options_button = OptionsButton(self.all_sprites)
    ...
    def run(self):
        while self.running:
            for event in pygame.event.get():
                ...
                self.all_sprites.draw(self.screen)
                if self.is_character_editor:
                    self.screen.blit(character_editor.img1, (width // 2.34718826406, height // 1.67962674961))
                    self.screen.blit(character_editor.img2, (width // 1.96721311475, height // 1.51472650771))
                    self.screen.blit(character_editor.img3, (width // 2.09378407852, height // 1.37931034483))
                    self.screen.blit(character_editor.img4, (width // 1.9452887538, height // 1.26909518214))
                    character_editor.hint_text_blit(width, height)
                    self.screen.blit(character_editor.line_text, (width // 11.2280701754, height // 3.85714285714))
                    if character_editor.is_enter:
                        if event.type == pygame.KEYDOWN:
                            ...
                            character_editor.name_text = character_editor.font2.render(character_editor.name,
                                                                                       True, character_editor.font_color)
                    self.screen.blit(character_editor.name_text, (width // 11.2280701754, height // 3.49514563107))
                if self.is_options:
                    with open('what_definition.txt', mode='r', encoding='utf-8') as file:
                        if file.read() == '1920':
                            k = 1
                        ...
                        img_resolution = font.render('Разрешение:', True, (146, 107, 56))
                        img_music = font.render('Музыка:', True, (146, 107, 56))
                    self.screen.blit(img_resolution, (width // 2.86995515695, height // 3.6))
                    self.screen.blit(img_music, (width // 2.86995515695, height // 2.66666666667))
