...
enemies = [[[] for _ in range(MAP_WIDTH * 2)] for _ in range(MAP_HEIGHT * 2)]
monsters = []
items = []
collision_objects = []
...
new_start_x = 20000
new_start_y = 22000
hero = Hero(new_start_x, new_start_y, 80, 124, 20, 8)
print(f"Name: {hero.name}")
print(f"Strength: {hero.strength}")
print(f"Endurance: {hero.endurance}")
print(f"IQ: {hero.iq}")
print(f"Body Type: {hero.body_type}")
...
slime = Slime(482, 541)
enemies[slime.x][slime.y].append(slime)
monsters.append(enemies[slime.x][slime.y][-1])
...
items.append(Item(140, 30, 60, 60, 'Шлем', os.path.join("pics", "helmet3.png")))
    ...
    if not is_inventory:
        ...
        for row in range(start_col - 15, end_col + 15):
            for col in range(start_row - 15, end_row + 15):
                ...
                if object and object.y * TILE_SIZE + scaled_offset_y < hero.camera_y - 200:
                    screen.blit(object.image, (object.x * TILE_SIZE + scaled_offset_x, object.y * TILE_SIZE + scaled_offset_y))
                
                for enemy in enemies[row][col]:
                    if enemy and enemy.y * TILE_SIZE + scaled_offset_y < hero.camera_y + 10:
                        ...
                        enemies[row][col].remove(enemy)
                        enemies[int(enemy.x)][int(enemy.y)].append(enemy)
